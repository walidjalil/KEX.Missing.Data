...
imp.method <- append(imp.method,"pmm")                            # Append the vector with pmm (predictive mean matching)
...
for (loop in c(1:1)) {
  ...
  HCC.imp <- mice(HCC.test, m=40, maxit = 40, method = imp.method) 
  
  for (r in c(1:40)) {
  #rand.ini <- sample(1:165, 15, replace = FALSE)
  print(rand.ini)
  imputedfinished <- complete(HCC.imp,r)
