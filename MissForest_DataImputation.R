...
library(UBL)
library(caret)
# ------- Convert the dataframe to as.matrix then as.data.frame   once that is done
# remember to convert categorical data to as.factor ..otherwise random forest will not work!  <--- OBS OBS
...
HCC_R <- read.csv("~/HCC_R.txt", header=FALSE)
HCC_t <- HCC_R
cols <- sprintf("V%s", 1:23)  # create vector of column names.
HCC_t[cols] <- lapply(HCC_t[cols], factor)   # turn columns into factors. !!
...
HCC.imp <- missForest(HCC_t, maxiter = 30)

#rand.ini <- sample(1:165, 15, replace = FALSE)
print(rand.ini)
imputedfinished <- HCC.imp$ximp
...
kmeans <- KMeans_rcpp(data = data.imp, 7, num_init = p, max_iters = 10,
...
data.imp$V51 <- factor(data.imp$V51,
                       ...
                       labels = c("One", "Two", "Three", "Four", "Five", "Six", "Seven")) # gör om faktorerna till "namn"
...
CBSO <- function(data, clusters, k=7) {
  if (k < 20) {
    data.balanced <- SmoteClassif(V51~., data.imp, C.perc = list(One = max(obs.percluster)/obs.percluster[1], Two = max(obs.percluster)/obs.percluster[2], Three = max(obs.percluster)/obs.percluster[3], Four = max(obs.percluster)/obs.percluster[4], Five = max(obs.percluster)/obs.percluster[5], Six = max(obs.percluster)/obs.percluster[6], Seven = max(obs.percluster)/obs.percluster[7]), k, repl = FALSE,
                                  dist = "HEOM")
    cat("j = ", j)  
    j <<- j + 1
